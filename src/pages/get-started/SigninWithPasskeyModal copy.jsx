...
function SigninWithPasskeyModal() {
  const { passkeyModalIsOpen, setPasskeyModalIsOpen } = useAuth();
  ...
  return (
    <>
      <Button
        ...
        className="h-12 rounded-2xl bg-[#101828] px-5 text-sm font-semibold text-white shadow-sm hover:bg-[#1D2939]"
      >
      ...
      {passkeyModalIsOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#101828]/45 px-4 backdrop-blur-sm">
          <div className="relative w-full max-w-[460px] rounded-[28px] border border-white/70 bg-white p-6 shadow-2xl">
            <button
              type="button"
              onClick={() => !isLoading && setPasskeyModalIsOpen(false)}
              disabled={isLoading}
